      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <template v-if="step === 1">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- No. Telepon -->
            <div class="space-y-1.5">
              <Label for="phone" class="text-sm font-medium">No. Telepon <span class="text-red-500">*</span></Label>
              <div class="relative">
          ...
          <!-- Alamat -->
          <div class="space-y-1.5">
            <Label for="address" class="text-sm font-medium">Alamat <span class="text-red-500">*</span></Label>
            <div class="relative">
        ...
        <template v-else-if="step === 2">
          ...
          <div class="space-y-1.5">
            <Label for="institution" class="text-sm font-medium">Institusi / Universitas <span class="text-red-500">*</span></Label>
            <div class="relative">
          ...
          <div class="grid grid-cols-2 gap-4">
            <!-- Departemen -->
            <div class="space-y-1.5">
              <Label for="department" class="text-sm font-medium">Departemen <span class="text-red-500">*</span></Label>
            ...
            <!-- Posisi -->
            <div class="space-y-1.5">
              <Label for="position" class="text-sm font-medium">Posisi <span class="text-red-500">*</span></Label>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="space-y-1.5 flex flex-col">
              <Label for="startDate" class="text-sm font-medium">Tanggal Mulai <span class="text-red-500">*</span></Label>
              <Popover>
                <PopoverTrigger as-child>
                  <Button
                    variant="outline"
                    :class="cn('w-full justify-start text-left font-normal px-3', !form.startDate && 'text-muted-foreground')"
                  >
                    <CalendarIcon class="mr-2 h-4 w-4 shrink-0" />
                    <span class="truncate">{{ form.startDate ? format(parseISO(form.startDate), 'dd MMM yyyy', { locale: id }) : "Pilih Tanggal" }}</span>
                  </Button>
                </PopoverTrigger>
            ...
            <!-- Tanggal Selesai -->
            <div class="space-y-1.5 flex flex-col">
              <Label for="endDate" class="text-sm font-medium">Tanggal Selesai <span class="text-red-500">*</span></Label>
              <Popover>
                <PopoverTrigger as-child>
                  <Button
                    variant="outline"
                    :class="cn('w-full justify-start text-left font-normal px-3', !form.endDate && 'text-muted-foreground')"
                  >
                    <CalendarIcon class="mr-2 h-4 w-4 shrink-0" />
                    <span class="truncate">{{ form.endDate ? format(parseISO(form.endDate), 'dd MMM yyyy', { locale: id }) : "Pilih Tanggal" }}</span>
                  </Button>
                </PopoverTrigger>
                <PopoverContent class="w-auto p-0" align="start">
                  <Calendar
        ...
        <template v-else>
          ...
          <div class="space-y-1.5">
            <Label for="agreementFile" class="text-sm font-medium">Upload Surat Perjanjian (PDF) <span class="text-red-500">*</span></Label>
            <div v-if="form.agreementFile" class="flex items-center justify-between p-3 border rounded-md bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800">
              <div class="flex items-center gap-2 overflow-hidden">
                <FileText class="h-5 w-5 text-primary flex-shrink-0" />
                <a 
                  v-if="typeof form.agreementFile === 'string'"
                ...
                <span v-else class="text-sm font-medium truncate">
                  {{ form.agreementFile.name }}
                </span>
              </div>
              <Button type="button" variant="ghost" size="icon" @click="form.agreementFile = ''" class="h-8 w-8 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30">
                <X class="h-4 w-4" />
              </Button>
            </div>
            <template v-else>
              <Input
                ...
                required
              />
              <p class="text-xs text-slate-500">Maks. 1MB</p>
            </template>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Konfirmasi Password -->
            <div class="space-y-1.5">
              <Label for="confirmPassword" class="text-sm font-medium">Konfirmasi <span class="text-red-500">*</span></Label>
              <div class="relative">
                <Lock class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <Input
                  id="confirmPassword"
          ...
          <div class="flex gap-2">
            ...
            <Button
              type="submit"
              class="flex-1"
              :disabled="loading || !agreedToTerms || form.password !== form.confirmPassword || form.password.length < 8 || !form.agreementFile"
            >
          ...
          <!-- Validation Helper -->
          <div v-if="(!agreedToTerms || form.password !== form.confirmPassword || form.password.length < 8 || !form.agreementFile) && !loading" class="text-xs space-y-1 p-3 bg-slate-50 dark:bg-slate-900 rounded-md border border-slate-100 dark:border-slate-800">
...
import {
  ...
  ArrowRight, ArrowLeft, Check, Info, Calendar as CalendarIcon, FileText, X
...
const {
  step, showPassword, showConfirmPassword, agreedToTerms,
  form, startDateValue, endDateValue, handleFileChange,
  goToStep2, goToStep3, handleSubmit, loading
} = useRegisterForm()
</script>
