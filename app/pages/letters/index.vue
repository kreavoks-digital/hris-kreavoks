    ...
    <Card class="border border-border bg-card overflow-hidden rounded-3xl">
      <Table>
        ...
        <TableBody>
          ...
          <template v-else-if="letters.length === 0">
            <TableRow>
          ...
          <template v-else>
            <TableRow v-for="letter in letters" :key="letter.id" class="border-border hover:bg-muted/50 transition-colors">
...
const openAdd = () => {
  modalMode.value = 'add';
  formData.value = { id: null, title: '', type: '', fileUrl: '' };
