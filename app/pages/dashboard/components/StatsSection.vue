<template>
  <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
    ...
    <!-- Sisa Masa Kerja Card (Masking Cutout Design) -->
    <div class="relative min-h-[100px]">
      <!-- Background with Circular Mask (The 'Bite') -->
      <div 
        class="absolute inset-0 bg-kv-primary/5 dark:bg-kv-primary/10 backdrop-blur-md border border-kv-primary/10 rounded-2xl md:rounded-3xl"
        style="mask-image: radial-gradient(circle at calc(100% - 3.2rem) 50%, transparent 48px, black 49px); -webkit-mask-image: radial-gradient(circle at calc(100% - 3.2rem) 50%, transparent 48px, black 49px);"
      >
        <div class="absolute -right-4 -top-4 w-24 h-24 bg-kv-primary/20 rounded-full blur-2xl"></div>
      </div>
      
      <!-- Text Content -->
      <div class="relative z-10 flex h-full items-center p-4 pr-[100px]">
      ...
      <!-- Doughnut Chart (Placed perfectly inside the cutout hole) -->
      <div class="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 w-[72px] h-[72px] md:w-20 md:h-20 shrink-0 flex items-center justify-center group cursor-pointer z-20">
        <!-- Tooltip on hover -->
        ...
        <svg viewBox="0 0 36 36" class="w-full h-full transform -rotate-90">
          ...
          <!-- Hadir (Foreground) -->
          <path
            class="text-kv-primary drop-shadow-sm transition-all duration-700 ease-out group-hover:stroke-[5]"
